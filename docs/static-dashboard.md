...
matching code. The exact CSV is also the full-data download.
...
`party_ref` values remain in downloads. Missing recorded affiliation remains
separate from government role. Verbeek’s agreed provisional-retention note is
visible in the reader and preserved in both kinds of CSV download.

## Interface and interpretation

Shared filters apply to the timeline, results, coding matrix, and selection
export. Multiple values within a field use OR; different fields use AND. Facet
...
`npm test`: all 11 checks pass. Coverage includes:

- Exact upstream CSV digest and byte-identical full-data download.
...
- CSV exports round-trip quoted fields, line breaks, evidence and full text.
- Code-label and matrix text contrast.
- Component rendering for filters, reader, method/data pages, exceptional records,
  and populated/empty charts.
